document.addEventListener('DOMContentLoaded', function() {
        ...
        if (dayEvents.length === 0) {
            ...
            dayEvents.forEach(ev => {
                ...
                html += `
                    ...
                    <div class="digest-card" onclick="showAptDetailsById('${ev.id}')">
    ...
    window.switchAptView = function(view) {
        ...
        const createHeader = document.getElementById('iosCreateHeader');
        const searchHeader = document.getElementById('iosSearchHeader');
    ...
    window.openAptSubModal = function(id) { document.getElementById(id).style.display = 'flex'; };
    window.closeAptSubModal = function(id) { document.getElementById(id).style.display = 'none'; };
    ...
    window.respondApt = async function(status) {
        const res = await fetch(`/appointments/api/respond/${currentInvitationId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
            body: JSON.stringify({ status })
        });
        if ((await res.json()).ok) {
            closeAptSubModal('aptViewModal');
            calendar.refetchEvents();
        }
    }

    window.deleteApt = async function() {
        if (!confirm('Cancel this appointment?')) return;
        const res = await fetch(`/appointments/api/destroy/${currentEventId}`, {
            method: 'DELETE',
    ...
    function showAptDetails(event) {
        ...
        document.getElementById('viewAptTitle').innerText = event.title;
        document.getElementById('viewAptTime').innerText = event.start.toLocaleString() + ' - ' + (event.end ? event.end.toLocaleString() : '');
        document.getElementById('viewAptCreator').innerText = props.creator;
        
        const desc = props.description;
        const descContainer = document.getElementById('viewAptDescContainer');
        if (desc) {
            document.getElementById('viewAptDesc').innerText = desc;
            descContainer.style.display = 'block';
        } else {
            descContainer.style.display = 'none';
        }
        
        const actions = document.getElementById('aptInviteActions');
        const owned = document.getElementById('aptOwnerActions');
        const badge = document.getElementById('viewAptStatus');
        ...
        if (props.type === 'owned') {
            ...
            currentInvitationId = event.id.replace('inv-', '');
            if (props.status === 'pending') {
                actions.style.display = 'flex';
            } else {
                badge.innerText = 'Status: ' + props.status;
                badge.style.color = props.status === 'accepted' ? '#1cc88a' : '#e74a3b';
            }
        } else if (props.type === 'holiday') {
            badge.innerText = 'Public Holiday';
            badge.style.color = '#1cc88a';
        }
        
        openAptSubModal('aptViewModal');
    }
