          ...
          <div class="info-card">
            ...
            <div class="affirmation-preview">
              @if($currentAffirmation)
                <div class="affirmation-preview-quote">"{{ $currentAffirmation->quote }}"</div>
                <div class="affirmation-preview-meta">
                  @if($currentAffirmation->author)
                    <span>Author: {{ $currentAffirmation->author }}</span>
                  @endif
                  <span class="status-pill live">{{ $currentAffirmation->display_status }}</span>
                  @if($currentAffirmation->publish_at)
                    <span>Posted: {{ $currentAffirmation->publish_at->format('M d, Y g:i A') }}</span>
                  @endif
                </div>
              @else
            ...
            <form action="{{ route('admin.daily-affirmations.store') }}" method="POST">
              ...
              <div class="form-grid">
                <div class="form-group">
                  ...
                  <textarea name="quote" id="quote" class="form-control" rows="4" required placeholder="Write the affirmation quote here...">{{ old('quote') }}</textarea>
                </div>

                <div class="form-group">
                  <label for="author">Author or Source (Optional)</label>
                  <input type="text" name="author" id="author" class="form-control" value="{{ old('author') }}" placeholder="e.g. AskDocPH, Maya Angelou">
                </div>
                ...
                <div class="form-group">
                  ...
                  <select name="publish_state" id="publish_state" class="form-control" onchange="toggleScheduleField(this, 'createScheduleWrap')">
                    <option value="draft" {{ old('publish_state', 'draft') === 'draft' ? 'selected' : '' }}>Save as Draft</option>
                    <option value="publish_now" {{ old('publish_state') === 'publish_now' ? 'selected' : '' }}>Post Today / Post Now</option>
                    <option value="scheduled" {{ old('publish_state') === 'scheduled' ? 'selected' : '' }}>Schedule for Specific Day and Time</option>
                  </select>
                </div>

                <div class="form-group" id="createScheduleWrap" style="display: {{ old('publish_state') === 'scheduled' ? 'flex' : 'none' }};">
                  ...
                  <input type="datetime-local" name="scheduled_for" id="scheduled_for" class="form-control" value="{{ old('scheduled_for') }}">
                </div>
              </div>
            ...
            <table class="affirmation-table">
              ...
              <tbody>
                @forelse($affirmations as $affirmation)
                  ...
                  <tr>
                    <td class="quote-cell">
                      <div class="quote-text">"{{ $affirmation->quote }}"</div>
                      @if($affirmation->author)
                        <div class="quote-meta">Author: {{ $affirmation->author }}</div>
                      @endif
                    </td>
                    <td>
                      <span class="status-pill {{ $state }}">{{ $affirmation->display_status }}</span>
                      @if($isCurrent)
                    ...
                    <td>{{ optional($affirmation->creator)->short_name ?? 'Admin' }}</td>
                    <td>
                      <div class="table-actions">
                        @if($state === 'scheduled')
                        <form action="{{ route('admin.daily-affirmations.publish-now', $affirmation) }}" method="POST">
                          @csrf
                        ...
                        <form action="{{ route('admin.daily-affirmations.destroy', $affirmation) }}" method="POST" onsubmit="return confirm('Delete this affirmation?');">
                          @csrf
                          @method('DELETE')
