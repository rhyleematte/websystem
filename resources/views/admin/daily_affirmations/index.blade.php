...
<main class="dash">
  <div class="admin-body">
      <div class="glass-panel">
        ...
        {{-- Live Affirmation Glass Card --}}
        <div class="live-affirmation-card">
            <h3><i data-lucide="sparkles" style="color: #a855f7;"></i> Current Live Affirmation</h3>
            @if($currentAffirmation)
                <div class="affirmation-quote">"{{ $currentAffirmation->quote }}"</div>
                <div class="affirmation-meta">
                    <span class="status-pill live">{{ $currentAffirmation->display_status }}</span>
                    @if($currentAffirmation->author)
                        <span><i data-lucide="user" style="width:14px; margin-right:4px;"></i>{{ $currentAffirmation->author }}</span>
                    @endif
                    @if($currentAffirmation->publish_at)
                        <span><i data-lucide="clock" style="width:14px; margin-right:4px;"></i>Posted: {{ $currentAffirmation->publish_at->format('M d, Y g:i A') }}</span>
                    @endif
                </div>
            @else
                ...
                @forelse($affirmations as $affirmation)
                  ...
                  <tr>
                    <td style="max-width: 300px;">
                      <div style="line-height: 1.5; margin-bottom: 4px;">"{{ Str::limit($affirmation->quote, 80) }}"</div>
                      @if($affirmation->author)
                        <div style="color: rgba(255,255,255,0.4); font-size: 0.85rem;">- {{ $affirmation->author }}</div>
                      @endif
                    </td>
                    <td>
                    ...
                    <td style="color: rgba(255,255,255,0.6);">{{ optional($affirmation->creator)->short_name ?? 'Admin' }}</td>
                    <td>
                      <div style="display:flex; justify-content: flex-end; gap: 8px; align-items: center;">
                        ...
                        <button type="button" class="btn-action edit" data-edit='@json($editPayload)' onclick="openEditModal(this)">
                          <i data-lucide="edit-2" style="width:14px;"></i> Edit
                        </button>

                        <form action="{{ route('admin.daily-affirmations.destroy', $affirmation) }}" method="POST" onsubmit="return confirm('Delete this affirmation?');" style="margin:0;">
                          @csrf
                          @method('DELETE')
