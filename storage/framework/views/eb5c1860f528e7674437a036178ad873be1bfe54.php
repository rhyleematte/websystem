...
<?php $__env->startSection('title', 'Edit Resource – AskDocPH'); ?>
...
<?php $__env->startPush('styles'); ?>
  ...
  <style>
    ...
    .tool-select:hover { background: var(--hover); border-color: rgba(255,255,255,0.2); color: var(--text); }
    .tool-select:focus { border-color: rgba(124,58,237,0.6); }
    .tool-color-input {
      width: 34px;
      height: 34px;
      border: 1px solid var(--border);
    ...
    .existing-file-badge { margin: 0 20px 8px; display: flex; align-items: center; gap: 8px; font-size: 13px; color: var(--res-primary); }
  ...
  <div class="composer-wrap">
    <div class="composer-card">
      ...
      <div class="composer-header">
        ...
        <a href="<?php echo e(route('resources.show', $resource->id)); ?>" class="composer-discard" title="Cancel">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </a>
      </div>

      
      <form action="<?php echo e(route('resources.update', $resource->id)); ?>" method="POST" enctype="multipart/form-data" id="resourceForm" accept-charset="utf-8">
        <?php echo csrf_field(); ?>
        ...
        <div class="composer-meta">
          ...
          <div class="composer-field">
            ...
            <input type="text" name="title" class="composer-field-input" required maxlength="50"
                   value="<?php echo e($resource->title); ?>" placeholder="Resource title (max 50 chars)">
          </div>
          ...
          <div class="composer-field-row">
            <div class="composer-field">
              ...
              <select name="type" id="resTypeSelect" class="composer-field-select" required>
                <option value="Article" <?php echo e($resource->type === 'Article' ? 'selected' : ''); ?>>📄 Article</option>
                <option value="Audio" <?php echo e($resource->type === 'Audio' ? 'selected' : ''); ?>>🎵 Audio</option>
                <option value="Workbook" <?php echo e($resource->type === 'Workbook' ? 'selected' : ''); ?>>📓 Workbook</option>
                <option value="Media" <?php echo e($resource->type === 'Media' ? 'selected' : ''); ?>>🖼️ Media</option>
                <option value="Video" <?php echo e($resource->type === 'Video' ? 'selected' : ''); ?>>🎬 Video</option>
              </select>
            </div>
            ...
            <div class="composer-field">
              ...
              <input type="text" name="duration_meta" class="composer-field-input"
                     value="<?php echo e($resource->duration_meta); ?>" placeholder="e.g. 5 min read">
            </div>
          </div>
          ...
          <div class="composer-field">
            ...
            <input type="text" name="description" class="composer-field-input" required
                   value="<?php echo e($resource->description); ?>" placeholder="Short description shown on the card...">
          </div>
          ...
          <div class="composer-field">
            ...
            <input type="text" name="hashtags" class="composer-field-input"
                   value="<?php echo e($resource->hashtags); ?>" placeholder="anxiety, coping, stress (comma separated)">
          </div>
        </div>
        ...
        <div class="existing-file-badge" style="display:flex;align-items:center;gap:10px;">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          <span>Current file: <strong><?php echo e(strtoupper($resource->file_type)); ?></strong> attached — upload a new one below to replace it.</span>
          <button type="button" class="chip-btn" style="margin-left:auto;" onclick="markResourceFileForRemoval()">
            <i data-lucide="trash-2"></i> Remove file
          </button>
        </div>
        <?php endif; ?>
        ...
        <div class="composer-field">
          ...
          <div style="display:flex;align-items:center;gap:10px;flex:1;flex-wrap:wrap;">
            ...
            <span id="thumbFileName" style="font-size:13px;color:var(--muted);">
              <?php echo e($resource->thumbnail ? 'Current: '.basename($resource->thumbnail) : 'No image selected'); ?>
        ...
        <?php if($resource->thumbnail): ?>
          <img id="thumbPreview" src="<?php echo e($resource->thumbnail_url); ?>" style="display:block; max-height:80px; border-radius:8px; border:1px solid var(--border); margin:8px 20px; object-fit:cover;">
        <?php else: ?>
        ...
        <div class="composer-body">
          ...
          ?>
          <div id="quill-editor"><?php echo $safeContent; ?></div>
        </div>
        ...
        <div class="composer-toolbar" id="composerToolbar">
          <div class="toolbar-left">
            ...
            <select id="fontSelect" class="tool-select" data-tip="Font" onchange="quillFont(this.value)">
            ...
            <select id="sizeSelect" class="tool-select" data-tip="Font size" onchange="quillSize(this.value)">
...
<div id="mediaResizeBar">
  ...
  <div class="mrb-section">
    ...
    <button class="mrb-delete" onclick="deleteSelectedMedia()" title="Delete">
...
const quill = new Quill('#quill-editor', {
  theme: 'snow',
  placeholder: 'Update your resource content...',
...
const initialContent = <?php echo json_encode($resource->content); ?>;
quill.root.innerHTML = initialContent;

function quillFont(font) {
  if (!font) font = false;
  quill.format('font', font);
  updateActiveStates();
}

function quillSize(size) {
  if (!size) size = false;
...
// ── Keyboard shortcuts for selected media (Delete/Backspace) ──
// If a media element is selected via the resize toolbar, allow Delete/Backspace to remove it.
...
function updateActiveStates() {
  ...
  if (fontSelect) fontSelect.value = fmt.font || '';
  const sizeSelect = document.getElementById('sizeSelect');
  if (sizeSelect) sizeSelect.value = fmt.size || '';
  ...
  if (fmt.align === 'left') document.getElementById('btnAlignLeft')?.classList.add('active');
  else if (fmt.align === 'center') document.getElementById('btnAlignCenter')?.classList.add('active');
  else if (fmt.align === 'right') document.getElementById('btnAlignRight')?.classList.add('active');
  else if (fmt.align === 'justify') document.getElementById('btnAlignJustify')?.classList.add('active');
}
...
function removeChip(btn, name) {
  ...
  if (fileInputEl && fileInputEl.files && fileInputEl.files[0] && fileInputEl.files[0].name === name) {
    fileInputEl.value = '';
    if (removeFileInput) removeFileInput.value = '0';
  }
}
...
function uploadAttachedFile(file, callback) {
  ...
  formData.append('resource_id', '<?php echo e($resource->id); ?>');

  fetch('<?php echo e(route("resources.upload-media")); ?>', {
    method: 'POST',
    body: formData,
  })
    .then(res => {
...
function markResourceFileForRemoval() {
  if (!removeFileInput) return;
  removeFileInput.value = '1';
  if (fileInput) fileInput.value = '';

  const badge = document.querySelector('.existing-file-badge');
  if (badge) {
    const text = badge.querySelector('span');
    if (text) {
      text.textContent = 'Existing file will be removed when you save. Upload a new one to replace it.';
    }
    badge.style.opacity = '0.7';
  }

  toast('🗑️ Existing file will be removed when you save.');
}
...
document.getElementById('videoInput').addEventListener('change', function(e) {
  ...
  const range = quill.getSelection(true);
  const idx = range ? range.index : quill.getLength();
  quill.insertEmbed(idx, 'htmlVideo', url);
  ...
  if (ext === 'pdf') {
    ...
    reader.onload = async function() {
      try {
        ...
        // Insert placeholder link for the PDF
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\websystem\resources\views/resources/edit.blade.php ENDPATH**/ ?>